...
export default function VentanaTicketPreview() {
  const { id } = useLocalSearchParams<{ id: string }>()
  const router = useRouter()

  const { data: ticket, isLoading, isError, refetch } = useQuery({
    queryKey: ['tickets', 'detail', 'ventana', id],
  ...
  if (isError || !ticket) {
    return (
      <YStack f={1} ai="center" jc="center" bg="$background" p="$4">
        ...
        <Button mt="$2" variant="outlined" onPress={() => router.replace('/ventana/tickets')}>Volver</Button>
      </YStack>
    )
  }
  ...
  return (
    <ScrollView flex={1} backgroundColor="$background">
      <YStack p="$4" gap="$4" ai="center">
        <XStack gap="$2">
          <Button variant="ghost" onPress={() => router.replace('/ventana/tickets')}>Cerrar</Button>
